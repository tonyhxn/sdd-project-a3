...
router.post('/marketprice', async (req, res) => {
    try {
        ...
        await console.log(`Retrieving price for ${item_id} from database...`)
        ...
        await console.log(`Retrieved ${item_id}: marketprice [${market_price}] from database`)
        res.send({market_price: market_price}) // completing request by sending market_price as response
        await console.log(`Returned ${item_id}: marketprice [${market_price}] to frontend response`)
    } catch (error) {
